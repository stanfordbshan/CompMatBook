...
#     x = rdfdata[i][:, 0]
#     y = rdfdata[i][:, 1]
#     ax3.plot(x, y, label=label, linestyle=line_styles[i % 4])

colors = ['black', 'blue']  # Black and Blue colors
line_styles = ['-', '--']  # Solid and Dashed line styles
...
for i, T in enumerate([2.5, 1000, 1500, 2000]):
    ...
    # Choose color and line style based on index i
    chosen_color = colors[i % 2]
    chosen_line_style = line_styles[(i // 2) % 2]
    
    ax3.plot(x, y, label=label, linestyle=chosen_line_style, color=chosen_color)

ax3.legend(loc='upper left', prop={'size':12}, frameon=False)
plt.xlim([0,8.5])
plt.ylim([0,8])
